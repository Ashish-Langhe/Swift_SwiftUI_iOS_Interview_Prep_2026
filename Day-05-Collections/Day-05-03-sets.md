...
For a senior iOS engineer, the topic **Sets** is evaluated through this lens: collections are data-structure decisions; senior engineers choose them from access pattern, identity, order, and mutation cost. The goal is not to memorize the keyword or syntax in isolation. The goal is to understand how this topic changes correctness, ownership, testability, performance, and team-scale maintainability.
...
These examples are intentionally small, but they are shaped like real app code. Use them to connect **Sets** to code you might write in a SwiftUI/UIKit feature.
...
### Track uniqueness and membership

```swift
var selectedIDs: Set<Product.ID> = []
selectedIDs.insert(product.id)
selectedIDs.remove(product.id)
```

Sets make selection, favorites, and deduplication straightforward.
