...
For a senior iOS engineer, the topic **Nested Functions** is evaluated through this lens: functions are API design units; labels, return types, async/throws, and side effects define how other engineers use the code. The goal is not to memorize the keyword or syntax in isolation. The goal is to understand how this topic changes correctness, ownership, testability, performance, and team-scale maintainability.
...
For **Nested Functions**, fill it like this:
...
These examples are intentionally small, but they are shaped like real app code. Use them to connect **Nested Functions** to code you might write in a SwiftUI/UIKit feature.
...
### Hide helper logic inside a larger operation

```swift
func cleanedSearchQuery(_ query: String) -> String {
    func trim(_ value: String) -> String {
        value.trimmingCharacters(in: .whitespacesAndNewlines)
    }

    return trim(query).lowercased()
}
```

Nested helpers are useful when no other scope should call the helper directly.

### Why This Fits Nested Functions
